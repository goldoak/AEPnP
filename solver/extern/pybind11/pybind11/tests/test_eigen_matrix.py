...
def test_numpy_ref_mutators():
    ...
    m.reset_refs()  # In case another simulation already changed it
...
def test_nocopy_wrapper():
    ...
    # For the row-major simulation, we take a long matrix in row-major, so only the third is allowed:
    with pytest.raises(TypeError) as excinfo:
