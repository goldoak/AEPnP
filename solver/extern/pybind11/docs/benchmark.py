...
for codegen in [generate_dummy_code_pybind11, generate_dummy_code_boost]:
    ...
    for i in range(0, 10):
        nclasses = 2**i
        with open("simulation.cpp", "w") as f:
            f.write(codegen(nclasses))
        n1 = dt.datetime.now()
        os.system(
            "g++ -Os -shared -rdynamic -undefined dynamic_lookup "
            "-fvisibility=hidden -std=c++14 simulation.cpp -I include "
            "-I /System/Library/Frameworks/Python.framework/Headers -o simulation.so"
        )
        n2 = dt.datetime.now()
        elapsed = (n2 - n1).total_seconds()
        size = os.stat("simulation.so").st_size
